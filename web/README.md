...
Во вкладке «Видео» можно вставить ссылку — браузер сам скачает видео в память и обработает как
локальный файл.

- **Прямые ссылки на видео** (`.mp4`/`.webm`/`.mov`) с **CORS** — работают сразу, без бэкенда.
- **Dropbox / Google Drive** и прямые ссылки **без CORS** — нужен бесплатный **CORS-прокси**
  (Cloudflare Worker). Задеплойте его по инструкции в [`../cloudflare-proxy/`](../cloudflare-proxy/)
  (≈2 минуты, бесплатно) и вставьте адрес воркера в **Настройки → «Прокси для ссылок»**. После этого
  ссылки Dropbox/Google Drive открываются автоматически.
- **YouTube / TikTok / Instagram** — **не поддерживаются**: это не файлы, а страницы с зашифрованными
  потоками; им нужен `yt-dlp` на полноценном сервере. Скачайте видео и откройте файл как обычно.

> Прокси — это отдельная крошечная функция на вашем аккаунте Cloudflare; сам сайт остаётся статикой
> на GitHub Pages. CORS — защита браузера, обойти её чисто на клиенте нельзя, поэтому и нужен прокси.
